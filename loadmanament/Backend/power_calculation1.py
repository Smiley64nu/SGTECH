...
OL = 0.60  # Overload limit
RF = 0.15  # Reverse Flow limit
N = 0.81  # loss BESS
battery_size = 215  # kWh
battery_size_power = battery_size * 60 / 15  # kW
Power_bat_max = 150  # kW power ของ battery ที่จ่ายได้สูงสุดทุก 15 นาที
...
def calculate_power1(P_forecasting, timestamp, Psh):
...
# 🔹 ฟังก์ชันหลัก
def process_power_data1(df, Psh):
    results = []
    ...
    for i in range(len(df)):
        ...
        P_New, P_B_out, P_Bat_1 = calculate_power1(P_forecasting, timestamp, Psh)
...
def calculate_difference1(results):
...
def calculate_time_period1(starttimes, endtimes):
    time_period = []
